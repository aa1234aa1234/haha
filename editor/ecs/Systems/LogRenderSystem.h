...
public:
    ...
    void Update() {
		for(auto& p : entities) {
			...
			float offset = sc->GetComponent<ScrollableComponent>(p).offset;
			...
			{
				if (log.log.size() <= i || i < 0) break;
				Text t = Text{transform.position.x+10, transform.position.y+k-(offset-logs.x*ROW_HEIGHT), log.log[i]};
				TextHandler::getInstance()->manualDrawText(t, transform.position.x+transform.size.x, transform.position.y+transform.size.y, transform.position.x, transform.position.y);
			}
		}
    }
