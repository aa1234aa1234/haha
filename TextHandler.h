...
public:
	...
	void manualDrawText(Text& k, int widthLim=1e9, int heightLim = 1e9, int widthMin = -1e9, int heightMin = -1e9) {
		shader->use();
		//textBuffer->bind();
        ...
        for (auto& p : k.text) {
			...
			if(k.posx < widthMin || k.posy < heightMin || k.posx > widthLim || k.posy > heightLim) continue;
